...
#[derive(Clone)]
pub struct DchiquitoNaiveImpl;

lazy_static::lazy_static! {
    /// Name of the interpreter. Often a github repo or website name.
    static ref NAME: String = String::from("dchiquito/naive");
    /// Source Website
    ...
    /// Source folder
    static ref SRC_DIR: String = String::from("build/src/dchiquito/naive");
    /// Actual EXE ran.
    static ref RESULT_EXE: String = String::from("build/src/dchiquito/naive/target/release/bf-compiler");
}

impl BFImpl for DchiquitoNaiveImpl {
    fn name(&self) -> String {
        NAME.clone()
    }
    ...
    fn get(&self) {
        git_repo_branch(&URL, &SRC_DIR, "naive");
    }
